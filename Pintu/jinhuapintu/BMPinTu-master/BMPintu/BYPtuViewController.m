...
#import "FouctionView/BYNavView.h"
...
@interface BYPtuViewController () <UIGestureRecognizerDelegate, BYPromotViewtoReplayDelegate>
...
- (void)setUIWithBGImage {
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:self.view.frame];
    imageView.image = [UIImage imageNamed:@"bg_2"];
    [self.view addSubview:imageView];
    
    BYNavView *navV = [[BYNavView alloc] init];
...
//    [tipBtn setTitle:@"求救BG" forState:UIControlStateNormal];
//    [tipBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//    [tipBtn setImage:[UIImage imageNamed:@"ing_qiujiu"] forState:UIControlStateNormal];
    [self.view addSubview:tipBtn];
    [tipBtn setBackgroundImage:[UIImage imageNamed:@"ing_qiujiu"] forState:UIControlStateNormal];
    tipBtn.size = CGSizeMake(120, 40);
    tipBtn.centerX = logo.centerX;
    tipBtn.top = self.view.bottom-100;
    ...
    UIButton *newPintuBtn = [UIButton buttonWithType:UIButtonTypeSystem];
//    [newPintuBtn setTitle:@"开始拼图" forState:UIControlStateNormal];
    newPintuBtn.frame = tipBtn.frame;
    newPintuBtn.layer.cornerRadius = 5;
//    [newPintuBtn setBackgroundColor:[UIColor blackColor]];
//    [newPintuBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [newPintuBtn setBackgroundImage:[UIImage imageNamed:@"ing_kspt"] forState:UIControlStateNormal];
    newPintuBtn.top = tipBtn.bottom + 10;
    [newPintuBtn addTarget:self action:@selector(daluan) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:newPintuBtn];
    
    self.backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.backBtn setButtonWithFrame:CGRectMake(20, [UIApplication sharedApplication].statusBarFrame.size.height + 4, 60, 36) title:@"" titleColor:[UIColor blackColor] Target:self action:@selector(backtoLastView:)];
    ...
    _timerView.center = CGPointMake(self.view.right-50, self.backBtn.centerY);
//    _timerView.
...
- (void)getRandPintuImage {
    ...
    NSArray *imageNameArr = @[@"1", @"2", @"3", @"4", @"5"];
    
    NSInteger rand = arc4random()%5;
  
    _randImageName = imageNameArr[rand];
    
    _randImageName = [NSString stringWithFormat:@"img_%i", _randImageName.intValue];
}
...
- (void)initializeDataSet {
    ...
    self.tipsImgaeView.size = CGSizeMake([[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.width -120);
    self.tipsImgaeView.centerX = self.view.centerX;
    self.tipsImgaeView.centerY = self.view.centerY-100;
...
//    self.tipsImgaeView.image = [UIImage imageNamed:@"123"];
    ...
    // 切片数量
    self.pieceHCount = _level;
    self.pieceVCount = _level;
    // 切片尺寸
...
- (void)move:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        if ( fabs(mimagePoint.x - mImgView.center.x) <= kVerificationTolerance &&
            ...
            //判断当前是否完成
            if (_pointCorrectArr.count == _level*_level) {
...
- (void)showPromotView {
    ...
    _bgView = [[BYShadowBGView alloc] init];
    [self.view addSubview:_bgView];

    //需要做判断，当前成绩与存储的成绩对比
    NSNumber *saveBestScore = [[NSUserDefaults standardUserDefaults] valueForKey:[NSString stringWithFormat:@"%li", (long)_level]];
    int bestScore = saveBestScore.intValue;
//    NSLog(@">>>>>>>>>.%i" , bestScore);
    if (bestScore > _timerView.usedTime || bestScore == 0) {
        
        bestScore = _timerView.usedTime;
    }
    
    BYPromotView *pView =  [[[NSBundle mainBundle] loadNibNamed:@"BYPromotView" owner:nil options:nil] firstObject];
    pView.delegate = self;
    [pView.currentSco setText:[NSString stringWithFormat:@"%i", _timerView.usedTime]];
    
    [pView.bestSco setText:[NSString stringWithFormat:@"%i", bestScore]];
...
- (void)daluan {
    //移除数组数据
    [[BYGameCenterInt gameCenter] asyntoLoadScores:(int)_level];
    [_pointCorrectArr removeAllObjects];
    [self setNewTimer];
...
- (void)moveCubestoAnotherPlace {
    ...
    [imageViews setArray:self.pieceCoordinateRectArray];
    NSMutableArray *pieceTypes = [NSMutableArray arrayWithArray:self.pieceTypeArray];
    
    NSInteger mXAddableVal;
    NSInteger mYAddableVal;
    
    NSMutableArray *pointCounts = [NSMutableArray array];
    for (int i = 0;  i < _level; i++) {
        for (int t = 0; t < _level; t++) {
            CGPoint center = CGPointMake(self.view.size.width*t/_level+1.5, self.originalCatImage.size.height*i/(_level+1)+_tipsImgaeView.top);
    ...
    for (UIImageView *imgV in _pieceImageViewsArray) {
        ...
        mXAddableVal = ([[[pieceTypes objectAtIndex:randNum] objectForKey:@(PieceSideTypeLeft)] integerValue] == PieceTypeOutside)?self.deepnessV:0;
        mYAddableVal = ([[[pieceTypes objectAtIndex:randNum] objectForKey:@(PieceSideTypeTop)] integerValue] == PieceTypeOutside)?self.deepnessH:0;
        
        CGPoint origin = itemPoint;
        origin.x +=  10;
        origin.y += _tipsImgaeView.height+10;
        //判断宽高大小，限制位置
        imgV.origin = origin;
